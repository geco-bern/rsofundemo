...
r_squared <- as.numeric(sprintf("%.3f",cor(costant_whc$le,costant_whc$aet, use = "complete.obs")^2))
...
r_squared <- as.numeric(sprintf("%.3f",cor(variable_whc$le,variable_whc$aet, use = "complete.obs")^2))
slope <- as.numeric(sprintf("%.3f",fit$coefficients[2]))
intercept <- as.numeric(sprintf("%.3f",fit$coefficients[1]))
