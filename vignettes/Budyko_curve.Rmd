...
Download the file "zroot_cwd80.nc" and put in the data folder

```{r}
# updating whc
library(ncdf4)

root <- nc_open(here("data","zroot_cwd80.nc"))

zroot = ncvar_get(root, "zroot_cwd80")
lons = ncvar_get(root, "lon")
lats = ncvar_get(root, "lat")

geo <- output |>
  unnest(site_info) |>
  select(lon  , lat)

geo$sitename <- output$sitename # to check if something goes wrong

n <- 1 # paramter to select the slice to average
new_whc <- c()
...
for(i in 1:216){
  ...
  zroot_region = zroot[(lonid-n):(lonid+n), (latid-n):(latid+n)]
  whc_site = mean(as.numeric(zroot_region, na.rm=T))
  new_whc <- append(new_whc,whc_site)
}
adf$whc <- new_whc
...
```{r}
# plot to highlits the difference between old and new whc
old_whc <- output |>
  unnest(site_info) |>
  select(whc)
old_whc <- old_whc[[1]]
ggplot(data.frame(old = old_whc, new = new_whc), aes(x=old,y=new))+
  geom_point()
```

```{r}
# fit with the new whc
fit = nls(aet/prec_cond ~ (exp((whc/a)*(1-(pet/prec_cond)^-1))-1)/(exp((whc/a)*(1-(pet/prec_cond)^-1))-((pet/prec_cond)^-1)),
          data = adf,start = list(a = 23))
