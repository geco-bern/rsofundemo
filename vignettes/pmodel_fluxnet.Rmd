...
```{r eval=FALSE}
driver <- readRDS("~/data/FluxDataKit/v3.1/rsofun_driver_data_v3.1.rds")
```
...
ggsave(here("fig/missingdata.png"), width = 6, height = 4)
```

```{r out.width="400px", fig.retina = 1, echo=FALSE}
knitr::include_graphics(here("fig/missingdata.png"))
```
...
```{r eval=FALSE, echo=FALSE}
ggsave(here("fig/gpp_mod_obs_frpue.png"), width = 6, height = 4)
```

```{r out.width="400px", fig.retina = 1, echo=FALSE}
knitr::include_graphics(here("fig/gpp_mod_obs_frpue.png"))
```

The evaluation of simulated vs. observed trends or annual totals requires complete sequences of good-quality observations. This requires subsetting the observational data as demonstrated in the article [P-model at FLUXNET trends](./vignettes/pmodel_fluxnet_trends.Rmd)
